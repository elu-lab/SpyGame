...
def save_game_info(game_id: str, player_list, location: str, citizen_llm:str, spy_llm:str):
    import os
    txt_dir = "log/"+game_id+"/"
...
def init_csv(game_id:str):
    ...
    pd.DataFrame({"speaker": [], "role": [], "virtual_role": [],"raw_response":[],"formatted_response":[], "utterance":[], "trial":[]}).to_csv(csv_dir, index=False)
    return

def update_csv(game_id:str, speaker:str, role:str, virtual_role, raw_response:str, formatted_response:str,utterance:str, trial: int):
...
def set_agents(citizen_llm:str, spy_llm: str, virtual_role_list: List[str]):
    ...
    for i in range(1,8):
        if i==mafia_id:
            ...
            citizen_list.append(Citizen("Player"+str(i), citizen_llm, virtual_role_list[i-1]))
