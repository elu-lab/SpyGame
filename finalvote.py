...
class FinalVote:
    def __init__(self, game_id: str, host: Host, player_list: List[Player], chat_list: List[Chat], location: str):
    ...
    def run(self):
        ...
        update_csv(self.game_id, "host","host","host","","","All turns end. Final vote start.",0)
...
formatted_response: {formatted_response}""")
                ...
                try:
                    ...
                    continue
                if "voted" not in formatted_response:continue
                if not get_agent_by_name(formatted_response["voted"], self.player_list): continue
                break
            try:
                voted_player_name:str = formatted_response["voted"]
                voted_player: Player = get_agent_by_name(voted_player_name, self.player_list)
            except:
                raise RuntimeError(f"""raw_response:{raw_response}
formatted_response:{formatted_response}""")
            update_csv(self.game_id, player.name, player.role, player.virtual_role, raw_response, formatted_response, f"I vote for {voted_player_name}", trial+1)
            self.vote_list.append({"voter":player, "voted":voted_player})
        
        for vote in self.vote_list:
            voted_player = vote["voted"]
            self.chat_list.append(Chat(vote["voter"], f"I vote for {voted_player.name}"))
        ...
        if not vote_result:
            ...
            self.chat_list.append(Chat(self.host, f"{vote_result.name} got most votes."))
            update_csv(self.game_id,"host","host","host","","",f"{vote_result.name} got most votes.",0)
